...
export default function Reason({ id, img, text }) {
    const { currentReason, handleClick } = useContext(reasonsContext);
    return (
        <div onClick={() => handleClick(id)} className={`${currentReason === id ? "flex-1" : 'w-[10%]'} duration-300 cursor-pointer rounded-xl bg-white/10 grid place-items-center relative overflow-hidden`}>
